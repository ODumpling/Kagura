// <auto-generated />
using System;
using Kagura.Data;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Kagura.Data.Migrations
{
    [DbContext(typeof(KaguraDbContext))]
    [Migration("20260529141321_AddSourcePromptOverrides")]
    partial class AddSourcePromptOverrides
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "10.0.8");

            modelBuilder.Entity("Kagura.Core.Domain.AgentRun", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<Guid?>("AgentTaskId")
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("EndedAt")
                        .HasColumnType("TEXT");

                    b.Property<int?>("ExitCode")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Kind")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(0);

                    b.Property<int?>("ProcessId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("PromptText")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("StartedAt")
                        .HasColumnType("TEXT");

                    b.Property<int>("Status")
                        .HasColumnType("INTEGER");

                    b.Property<string>("TranscriptLogPath")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<Guid>("WorkItemId")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("AgentTaskId");

                    b.HasIndex("Kind");

                    b.HasIndex("WorkItemId");

                    b.ToTable("AgentRuns");
                });

            modelBuilder.Entity("Kagura.Core.Domain.AgentTask", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<string>("BranchName")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<bool>("IncludeInPullRequest")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(true);

                    b.Property<string>("LastFailureReason")
                        .HasColumnType("TEXT");

                    b.Property<int>("Order")
                        .HasColumnType("INTEGER");

                    b.Property<int>("RetryAttempts")
                        .HasColumnType("INTEGER");

                    b.Property<string>("ReviewNotes")
                        .HasColumnType("TEXT");

                    b.Property<int>("Status")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TEXT");

                    b.Property<Guid>("WorkItemId")
                        .HasColumnType("TEXT");

                    b.Property<string>("WorktreePath")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("WorkItemId");

                    b.ToTable("AgentTasks");
                });

            modelBuilder.Entity("Kagura.Core.Domain.AutoReviewInteraction", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<Guid>("AgentRunId")
                        .HasColumnType("TEXT");

                    b.Property<Guid?>("AgentTaskId")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("Prompt")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("RespondedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("Response")
                        .HasColumnType("TEXT");

                    b.Property<int>("Sequence")
                        .HasColumnType("INTEGER");

                    b.Property<Guid>("WorkItemId")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("AgentTaskId");

                    b.HasIndex("WorkItemId");

                    b.HasIndex("AgentRunId", "Sequence");

                    b.ToTable("AutoReviewInteractions");
                });

            modelBuilder.Entity("Kagura.Core.Domain.Source", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<string>("ConfigJson")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<bool>("Enabled")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime?>("LastSyncedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("LocalRepoPath")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("TEXT");

                    b.Property<int>("Type")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("Sources");
                });

            modelBuilder.Entity("Kagura.Core.Domain.SourcePromptOverride", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("PromptText")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<int>("Role")
                        .HasColumnType("INTEGER");

                    b.Property<Guid>("SourceId")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("SourceId", "Role")
                        .IsUnique();

                    b.ToTable("SourcePromptOverrides");
                });

            modelBuilder.Entity("Kagura.Core.Domain.WorkItem", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<bool>("AutoApproveTriage")
                        .HasColumnType("INTEGER");

                    b.Property<bool>("AutoReviewEnabled")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER")
                        .HasDefaultValue(true);

                    b.Property<string>("Body")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<string>("BranchName")
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("ClosedAt")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("ExternalId")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("TEXT");

                    b.Property<int>("GrillStatus")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Labels")
                        .HasColumnType("TEXT");

                    b.Property<string>("LastTriageError")
                        .HasColumnType("TEXT");

                    b.Property<string>("OriginalBody")
                        .HasColumnType("TEXT");

                    b.Property<string>("PullRequestUrl")
                        .HasColumnType("TEXT");

                    b.Property<bool>("RalphLoopActive")
                        .HasColumnType("INTEGER");

                    b.Property<string>("RalphLoopHaltReason")
                        .HasColumnType("TEXT");

                    b.Property<string>("RalphLoopWaitingReason")
                        .HasColumnType("TEXT");

                    b.Property<Guid>("SourceId")
                        .HasColumnType("TEXT");

                    b.Property<int>("Status")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("TriagedAt")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("Url")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("SourceId", "ExternalId")
                        .IsUnique();

                    b.ToTable("WorkItems");
                });

            modelBuilder.Entity("Kagura.Core.Domain.WorkItemComment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("TEXT");

                    b.Property<string>("Content")
                        .IsRequired()
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<int>("Role")
                        .HasColumnType("INTEGER");

                    b.Property<Guid>("WorkItemId")
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("WorkItemId", "CreatedAt");

                    b.ToTable("WorkItemComments");
                });

            modelBuilder.Entity("Kagura.Core.Domain.AgentRun", b =>
                {
                    b.HasOne("Kagura.Core.Domain.AgentTask", "AgentTask")
                        .WithMany("Runs")
                        .HasForeignKey("AgentTaskId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Kagura.Core.Domain.WorkItem", "WorkItem")
                        .WithMany("Runs")
                        .HasForeignKey("WorkItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AgentTask");

                    b.Navigation("WorkItem");
                });

            modelBuilder.Entity("Kagura.Core.Domain.AgentTask", b =>
                {
                    b.HasOne("Kagura.Core.Domain.WorkItem", "WorkItem")
                        .WithMany("Tasks")
                        .HasForeignKey("WorkItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("WorkItem");
                });

            modelBuilder.Entity("Kagura.Core.Domain.AutoReviewInteraction", b =>
                {
                    b.HasOne("Kagura.Core.Domain.AgentRun", "AgentRun")
                        .WithMany("Interactions")
                        .HasForeignKey("AgentRunId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Kagura.Core.Domain.AgentTask", "AgentTask")
                        .WithMany()
                        .HasForeignKey("AgentTaskId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Kagura.Core.Domain.WorkItem", "WorkItem")
                        .WithMany("AutoReviewInteractions")
                        .HasForeignKey("WorkItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("AgentRun");

                    b.Navigation("AgentTask");

                    b.Navigation("WorkItem");
                });

            modelBuilder.Entity("Kagura.Core.Domain.SourcePromptOverride", b =>
                {
                    b.HasOne("Kagura.Core.Domain.Source", "Source")
                        .WithMany("PromptOverrides")
                        .HasForeignKey("SourceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Source");
                });

            modelBuilder.Entity("Kagura.Core.Domain.WorkItem", b =>
                {
                    b.HasOne("Kagura.Core.Domain.Source", "Source")
                        .WithMany("WorkItems")
                        .HasForeignKey("SourceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Source");
                });

            modelBuilder.Entity("Kagura.Core.Domain.WorkItemComment", b =>
                {
                    b.HasOne("Kagura.Core.Domain.WorkItem", "WorkItem")
                        .WithMany("Comments")
                        .HasForeignKey("WorkItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("WorkItem");
                });

            modelBuilder.Entity("Kagura.Core.Domain.AgentRun", b =>
                {
                    b.Navigation("Interactions");
                });

            modelBuilder.Entity("Kagura.Core.Domain.AgentTask", b =>
                {
                    b.Navigation("Runs");
                });

            modelBuilder.Entity("Kagura.Core.Domain.Source", b =>
                {
                    b.Navigation("PromptOverrides");

                    b.Navigation("WorkItems");
                });

            modelBuilder.Entity("Kagura.Core.Domain.WorkItem", b =>
                {
                    b.Navigation("AutoReviewInteractions");

                    b.Navigation("Comments");

                    b.Navigation("Runs");

                    b.Navigation("Tasks");
                });
#pragma warning restore 612, 618
        }
    }
}
